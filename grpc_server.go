		...
		case *pb.ProcessingRequest_RequestHeaders:
			...
			for _, n := range h.RequestHeaders.Headers.Headers {
				...
				if n.Key == "user" && isPOST {
					...
					var callCount int = userCall[userName]
					userCall[userName] = callCount + 1
				...
				Response: &pb.CommonResponse{
					HeaderMutation: &pb.HeaderMutation{
						...
						SetHeaders: []*core.HeaderValueOption{
							...
							{
								Header: &core.HeaderValue{
									Key:   "api-call-count-[" + userName + "]",
									Value: strconv.Itoa(userCall[userName]),
								},
							},
							{
								Header: &core.HeaderValue{
									Key:   "api-call-count-[" + departName + "]",
									Value: strconv.Itoa(departCount),
								},
							},
						},
					},
